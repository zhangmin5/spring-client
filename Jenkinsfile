	...
	stages {
	  ...
	  stage('Deploy') {
		steps {
			...
			script {
                openshift.withCluster() {
                	  sh 'oc new-app \'openjdk18-openshift~https://github.com/remkohdev/spring-client\' --strategy=source --build-env=\'JAVA_MAIN_CLASS=hello.Application\' -l app=springclient'
                }
            }
		}
	  }
