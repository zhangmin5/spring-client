			...
			script {
                openshift.withCluster() {
                  withCredentials([usernamePassword(
                    ...
                    sh "oc login https://c100-e.us-south.containers.cloud.ibm.com:30403 --token=${PASSWORD}"
                    sh 'oc new-app --name springclient \'redhat-openjdk-18/openjdk18-openshift:1.6~https://github.com/remkohdev/spring-client\' --strategy=source --allow-missing-images --build-env=\'JAVA_MAIN_CLASS=hello.Application\''
                  }
                }
            }
		}
